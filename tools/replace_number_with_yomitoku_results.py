...
from yomitoku.export import save_csv, save_html, save_json, save_markdown
from yomitoku.export import convert_json, convert_csv, convert_html, convert_markdown
...
def replace_number_with_yomitoku_results(image:np.ndarray, result:DocumentAnalyzerSchema, output_path:str)->Image.Image:
    out = image.copy()
    ...
    for word in result.words:
        ...
        if re.match(r"[+\-0-9,.]+", word.content):
            [tl, tr, br, bl] = word.points
            font = ImageFont.truetype("Arial.ttf", size=36)
            draw.rectangle([tl[0], tl[1], br[0], br[1]], fill=(255, 255, 255))
            draw.text((tl[0], tl[1]), word.content, font=font, fill=(0, 0, 0))
...
def process_single_file(args, analyzer, path, format):
    ...
    for page, img in enumerate(imgs):
        ...
        replaced_image_ouput_path = os.path.join(args.outdir, f"{dirname}_{filename}_p{page + 1}_replaced.png")
        replace_number_with_yomitoku_results(img, result, replaced_image_ouput_path)
